...
const gamesPower = gamesList.map(game => {
    ...
    var power = 1;
    Object.values(maxes).forEach(theMax => power*=theMax);
    return power;
}).filter(theValue => theValue!==undefined).map(theValue => +theValue!!);

console.log(gamesPower);
console.log(gamesPower.reduce((prevVal, currVal)=> prevVal+currVal,0))
